...
export default function PostDetail({ auth, post, relatedPosts, seo, post_types, comments: initialComments }) {
    ...
    // Initial comments harusnya sudah berbentuk nested/tree dari backend jika ingin tampilan bertingkat
    // Jika backend masih mengirim flat list, Anda perlu membangun struktur tree di frontend
    ...
    // --- Fungsi rekursif untuk memperbarui komentar (sama seperti yang sudah dibahas sebelumnya) ---
    const updateCommentsRecursively = (currentComments, newComment) => {
        return currentComments.map(comment => {
            if (comment.id === newComment.parent_id) {
                return {
                    ...comment,
                    // Pastikan replies selalu array
                    replies: [...(comment.replies || []), newComment]
                };
            }
            if (comment.replies && comment.replies.length > 0) {
                // Panggil fungsi ini lagi untuk balasan
                const updatedReplies = updateCommentsRecursively(comment.replies, newComment);
                // Hanya buat objek baru jika ada perubahan di replies
    ...
    // --- Fungsi rekursif untuk menghapus komentar (sama seperti yang sudah dibahas sebelumnya) ---
    const removeCommentRecursively = (currentComments, idToRemove) => {
        return currentComments.filter(comment => {
            if (comment.id === idToRemove) {
                return false; // Hapus komentar ini
            }
            if (comment.replies && comment.replies.length > 0) {
                // Coba hapus dari balasan secara rekursif
                comment.replies = removeCommentRecursively(comment.replies, idToRemove);
            }
        ...
        try {
            const response = await fetch('/post/comments', {
                ...
                body: JSON.stringify({
                    ...
                    // Tambahkan replied_to_user_username jika ada balasan
                    replied_to_user_username: replyToUsername || null,
                }),
            });

            if (response.ok) {
                const newComment = await response.json();
                // Asumsi backend mengembalikan newComment dengan `parent_id` dan `replied_to_user_username` jika ini balasan
                // Contoh: { id: ..., comment: "isi komen", user_id: ..., post_id: ..., parent_id: ..., user_comment: { ... }, replied_to_user_username: "NamaUserDibales" }


                setComments(prevComments => {
                    if (replyToCommentId) {
                        // Gunakan fungsi rekursif untuk menambahkan balasan ke parent yang benar
                        return updateCommentsRecursively(prevComments, newComment);
                    } else {
                        // Jika ini komentar utama baru, tambahkan ke awal array utama
    ...
    // --- Component rekursif untuk menampilkan komentar ---
    const CommentItem = ({ comment, auth, handleReplyClick, handleDeleteComment }) => {
        const hasReplies = comment.replies && comment.replies.length > 0;
        const isCurrentUserComment = auth?.user && auth.user.id === comment.user_id;
        ...
        return (
            <div className="bg-primary-background p-4 rounded-lg shadow-sm">
                ...
                <p className="text-primary-text break-words">
                    {/* Tambahkan bagian ini untuk menampilkan nama user yang dibalas */}
                    {comment.replied_to_user_username && (
                        <span className="text-blue-500 dark:text-blue-400 font-semibold mr-1">
                            @{comment.replied_to_user_username}
                        </span>
                    )}
                ...
                {hasReplies && (
                    <div className="ml-12 mt-4 space-y-4 border-l-2 border-gray-300 dark:border-gray-700 pl-4">
                        {comment.replies.map(reply => (
                            <CommentItem // Panggil CommentItem secara rekursif
                                key={reply.id}
                                comment={reply}
                                auth={auth}
                        ...
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {relatedPosts.map((relatedPost) => (
                                <Link
                                    ...
                                    >
                                        <p>{truncateText(relatedPost.content, 70)}</p>
                                    </div>
                                </Link>
                            ))}
                        </div>
